...
def get_dealers(public_key, city):
    ...
    for item in root_items:
        if (
            item.get("type") == "dir"
            and normalize_text(item.get("name")) == normalize_text(city)
        ):
            city_item = item
            break
    ...
    if city_item is None:
        for item in root_items:
            ...
            for sub in sub_items:
                if (
                    sub.get("type") == "dir"
                    and normalize_text(sub.get("name")) == normalize_text(city)
                ):
                    city_item = sub
                    break
    ...
    image_items.sort(
        key=lambda x: (
            ...
            if "ORJ" in normalize_text(x.get("name"))
                ...
                0
                if "PLANOGRAM" in normalize_text(x.get("name"))
                else 1
            ),
...
if "app_password" not in st.secrets:
    st.error(
        "Kritik: Streamlit Secrets içine app_password eklenmemiş.\n\nÖrnek:\napp_password = 'SIFRENIZ'"
    )
    st.stop()
...
if not st.session_state.authenticated:
    ...
    if st.button("Giriş Yap", type="primary", use_container_width=True):
        if password == str(st.secrets["app_password"]):
...
with st.sidebar:
    ...
    # Statik kapasite yerine dinamik hesaplama:
    capacity = rows * cols
