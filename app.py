...
Şehir/Bayi Filtreli ve Ayarlanabilir Grid Sürümü
"""
...
# =========================================================
# DİNAMİK VE AYARLANABİLİR SLOT GEOMETRİSİ
# =========================================================
def grid_boxes(h, w, rows, cols, x_offset=0, y_offset=0, scale_w=1.0, scale_h=1.0):
    """Rafların tam oturması için kenar boşluklu ve ölçeklenebilir grid yapısı."""
    boxes = []
    
    # Raf alanını sınırlandıralım
    rack_w = int(w * scale_w)
    rack_h = int(h * scale_h)
    
    for row in range(rows):
        y1 = int(round(y_offset + row * rack_h / rows))
        y2 = int(round(y_offset + (row + 1) * rack_h / rows))
        for col in range(cols):
            x1 = int(round(x_offset + col * rack_w / cols))
            x2 = int(round(x_offset + (col + 1) * rack_w / cols))
...
# =========================================================
# ANA PLANOGRAM ANALİZİ (DİNAMİK VE ÖLÇEKLİ)
# =========================================================
def analyze_planogram(reference, field, rows, cols, x_offset, y_offset, scale_w, scale_h):
    h, w = reference.shape[:2]
    ...
    boxes = grid_boxes(h, w, rows, cols, x_offset, y_offset, scale_w, scale_h)
...
# =========================================================
# SIDEBAR (ŞEHİR, BAYİ FİLTRELERİ VE HİZALAMA AYARLARI)
# =========================================================
with st.sidebar:
    st.header("🏢 Konum ve Bayi Seçimi")
    
    # bayiler.xlsx dosyasını okuma denemesi
    df_bayi = None
    try:
        df_bayi = pd.read_excel("bayiler.xlsx")
    except Exception:
        pass

    selected_city = "Tümü"
    selected_dealer = "Manuel Bayi"

    if df_bayi is not None and not df_bayi.empty:
        cols_list = df_bayi.columns.tolist()
        # Otomatik kolon bulma (Sehir / İl ve Bayi / Müşteri vb.)
        city_col = next((c for c in cols_list if 'sehir' in c.lower() or 'il' in c.lower()), cols_list[0])
        dealer_col = next((c for c in cols_list if 'bayi' in c.lower() or 'musteri' in c.lower() or 'unvan' in c.lower()), cols_list[1] if len(cols_list) > 1 else cols_list[0])

        cities = ["Tümü"] + sorted(df_bayi[city_col].dropna().astype(str).unique().tolist())
        selected_city = st.selectbox("Şehir Seçin", cities)

        if selected_city != "Tümü":
            filtered_dealers = df_bayi[df_bayi[city_col].astype(str) == selected_city][dealer_col].dropna().astype(str).unique().tolist()
        else:
            filtered_dealers = df_bayi[dealer_col].dropna().astype(str).unique().tolist()

        selected_dealer = st.selectbox("Bayi / Nokta Seçin", filtered_dealers)
    else:
        st.info("💡 bayiler.xlsx bulunamadı / okunamadı. Manuel giriş aktif.")
        selected_dealer = st.text_input("Bayi Adı", value="Manuel Bayi")
    ...
    st.divider()
    st.subheader("🎯 Grid Hizalama Ayarları")
    x_offset = st.slider("X Ofset (Yatay Kaydırma)", -300, 300, 0, 5)
    y_offset = st.slider("Y Ofset (Dikey Kaydırma)", -300, 300, 0, 5)
    scale_w = st.slider("Genişlik Ölçeği (Scale W)", 0.5, 1.5, 1.0, 0.01)
    scale_h = st.slider("Yükseklik Ölçeği (Scale H)", 0.5, 1.5, 1.0, 0.01)
...
st.caption(f"Seçilen Bayi: **{selected_dealer}** ({selected_city})")
        ...
        if st.button("🔍 Analizi Başlat", type="primary", use_container_width=True):
            with st.spinner("Görsel işleniyor ve eşleştiriliyor..."):
                res_img, results, summary = analyze_planogram(
                    ...
                    x_offset=x_offset, y_offset=y_offset, scale_w=scale_w, scale_h=scale_h
                )
                st.session_state.result_img = res_img
                st.session_state.results = results
                st.session_state.summary = summary
                st.session_state.report = build_report(selected_dealer, results, summary, capacity)
